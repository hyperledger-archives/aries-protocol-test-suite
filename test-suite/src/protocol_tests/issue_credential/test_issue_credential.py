...
async def test_issuer_v1_0_issuer_initiated(backchannel, connection, cred_def1, handler):
    ...
    connection.route_module(_handler)
...
async def cred_def1(backchannel, config):
    ...
    await backchannel.issue_credential_v1_0_ledger_init(config['provider'])
...
@pytest.fixture
async def handler(config, connection, backchannel):
    """Common initialization for all tests in this file"""
    global _handler
    if not _handler:
        _handler = Handler()
        await _handler.setup(config['provider'])
    return _handler
